
<section class="site-hero overlay" data-stellar-background-ratio="0.5" style="background-image: url(img/slider_home.jpg);">
  <div class="container">
    <div class="row align-items-center site-hero-inner justify-content-center">
      <div class="col-md-8 text-center">

        <div class="mb-5 element-animate">
          <h1>Pico Tours</h1>
          <p>Enjoy sustainable tourism with local tour guides</p>
        </div>

        <form action="/search" method="get" class="form-inline element-animate" id="search-form">
          <label for="s" class="sr-only">Location</label>
          <input type="search" name="search" class="form-control form-control-block search-input" placeholder="Search city...">
          <button type="submit" class="btn btn-primary">Search</button>
        </form>

      </div>
    </div>
  </div>
</section>
<!-- END section -->
